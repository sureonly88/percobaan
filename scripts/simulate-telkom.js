#!/usr/bin/env node
/**
 * Simulasi pembayaran Telkom Telepon (multi-periode)
 *
 * Usage:
 *   node scripts/simulate-telkom.js
 *   LOKET_CODE=LYKN KASIR_USERNAME=yakin node scripts/simulate-telkom.js
 */
...
// Note: Telkom inquiry response strukturnya identik dengan payment response
const INQUIRY = {
  rc: "0000",
  rc_msg: "Sukses",
  tipe_pesan: "inquiry",
...
/** Format comma-separated YYYYMM list → "Jul 2011, Jun 2011, Mei 2011" */
function fmtPeriodeList(periodeStr) {
  const months = ["Jan","Feb","Mar","Apr","Mei","Jun","Jul","Agu","Sep","Okt","Nov","Des"];
  return periodeStr.split(",").map((p) => {
    p = p.trim();
    if (p.length < 6) return p;
    const year  = p.substring(0, 4);
    const month = parseInt(p.substring(4, 6), 10);
    return `${months[month - 1] || p.substring(4,6)} ${year}`;
  }).join(", ");
}
...
async function main() {
  ...
  console.log("╔══════════════════════════════════════════════════════════════╗");
  console.log("║         Simulasi Pembayaran Telkom Telepon                   ║");
  console.log("╚══════════════════════════════════════════════════════════════╝\n");
  ...
  try {
    ...
    const idpel       = d.idpel || INQUIRY.input1;
    const nama        = d.nama;
    const kodeProduk  = INQUIRY.kode_produk;  // "telkom-telepon"
    const idTrx       = INQUIRY.id_trx;
    const jumBill     = Number(d.jum_bill) || 1;
    const rpAmount    = Number(d.rp_amount);
    const rpAdmin     = Number(d.rp_admin);
    const rpTotal     = Number(d.rp_total);
    const periodeList = fmtPeriodeList(d.periode);
    ...
    console.log(`📞 No. Telepon : ${idpel}`);
    console.log(`📦 Produk      : ${kodeProduk}`);
    console.log(`📅 Jml Tagihan : ${jumBill} bulan`);
    console.log(`📅 Periode     : ${periodeList}`);
    ...
    console.log(`🔖 Ref Biller  : ${d.refnum}`);
    console.log("");
    ...
    const requestPayload = {
      ...
      bills: [{
        ...
        total: rpTotal, admin: rpAdmin, rpAmount,
        periode: d.periode, jumBill: d.jum_bill,
        refnum: d.refnum,
      }],
    ...
    const periodLabel = `${jumBill} Tagihan`;  // "3 Tagihan"
    ...
    await conn.execute(
      `INSERT INTO multi_payment_items
       ...
       VALUES (?, ?, 'LUNASIN', 'TELKOM', ?, ?, ?, ?, ?, ?, ?, ?, 'SUCCESS', ?, ?, 0, ?, NOW())`,
      [
    ...
    const events = [
      {
        ...
        severity: "INFO",
        message: `Simulasi inquiry Telkom Telepon untuk ${idpel} (${jumBill} bulan)`,
        payload: { idpel, kodeProduk, idTrx, jumBill, periode: d.periode, source: "simulasi" },
      },
      {
        event_type: "PAYMENT_REQUEST_CREATED",
        severity: "INFO",
        message: "Payment request Lunasin Telkom berhasil dibuat (simulasi)",
        payload: { billsCount: 1, totalPembayaran: rpTotal, periodeList },
      },
      {
    ...
    console.log(`  Ref Biller       : ${PAYMENT_RESPONSE.data.refnum}`);

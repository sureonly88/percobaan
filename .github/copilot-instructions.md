...
Selalu gunakan MCP agentmemory tools di setiap sesi chat untuk menjaga konteks project tetap akurat dan persisten.
Server berjalan di `http://localhost:3111`. Start server dengan `nohup agentmemory > /tmp/agentmemory.log 2>&1 &` jika belum jalan (gunakan nohup agar tidak mati saat terminal ditutup).

### Di awal setiap sesi
- Panggil `agentmemory_search` atau `agentmemory_list_sessions` untuk memuat konteks project.
- Cari dengan kata kunci: `portal-utilitas`, `pdam`, `payment`, `perubahan-terakhir`.
...
- Gunakan `mcp_agentmemory_memory_save` untuk menyimpan fakta/konteks baru
